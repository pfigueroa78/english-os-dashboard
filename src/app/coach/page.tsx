...
export default function CoachPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "coach", content: "Loading your English OS class plan..." },
  ]);
  ...
  const [currentUnit, setCurrentUnit] = useState(E2E_DEMO ? "Unit 1" : "");
  const [currentLesson, setCurrentLesson] = useState(E2E_DEMO ? "Business advice speaking practice" : "");
  const [studyUnit, setStudyUnit] = useState(E2E_DEMO ? "Unit 1" : "");
  ...
  useEffect(() => {
    ...
    if (E2E_DEMO) {
      setMessages([{ role: "coach", content: buildTodayClassMessage("Unit 1", "Business advice speaking practice") }]);
      return;
    }
  ...
  async function createWorkbook(kind: "grammar" | "vocabulary") {
    ...
    try {
      ...
      const workbook: Workbook = { title: data.title, fileUrl: data.fileUrl, exportUrl: data.exportUrl, generatedAt: data.generatedAt };
      ...
      setMessages((current) => [...current, { role: "coach", content: `Listo. Generé el Excel de ${isGrammar ? "gramática" : "vocabulario"} para ${unitLabel(unit)}.` }]);
    } catch (err) {
  ...
  async function sendAgentMessage(customMessage?: string) {
    ...
    if (E2E_DEMO) {
      setMessages((current) => [...current, { role: "user", content: `[${activeAgent.name}] ${message}` }, { role: "coach", content: `${activeAgent.name}\n\nModo demo: aquí aparecería la retroalimentación especializada.` }]);
      return;
    }
    setError("");
    setAgentError("");
    ...
    try {
      const response = await fetch("/api/english-os/agents", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ agentId: activeAgent.id, message }) });
      const data = await response.json();
      ...
      setMessages((current) => [...current, { role: "coach", content: `${data.agent?.name || activeAgent.name}\n\n${data.reply || "No response returned."}`, usage: data.usage }]);
    } catch (err) {
  ...
  async function sendMessage(customMessage?: string) {
    ...
    if (E2E_DEMO) {
      setInput("");
      setMessages((current) => [...current, { role: "user", content: message }, { role: "coach", content: "Modo demo: el profesor respondería aquí usando el contexto real de la clase." }]);
      return;
    }
    setError("");
    setInput("");
    ...
    try {
      const response = await fetch("/api/english-os/coach", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message, conversationHistory: messages.slice(-12) }) });
      const data = await response.json();
      ...
      setLastCost(data.usage?.estimatedCostUSD ?? null);
      setLastTokens(data.usage?.totalTokens ?? null);
      setMessages((current) => [...current, { role: "coach", content: data.reply || "No response returned.", usage: data.usage }]);
    } catch (err) {
  ...
  function requestResourcePractice(resource: DriveUnitResource) {
    const details = [`Title: ${resource.title}`, `Type: ${resource.type}`, resource.section ? `Section: ${resource.section}` : "", resource.page ? `Page: ${resource.page}` : "", resource.exercise ? `Exercise: ${resource.exercise}${resource.exercisePart || ""}` : "", `URL: ${resource.url}`].filter(Boolean).join("\n");
    ...
    return (
      <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-slate-100">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <a href={workbook.exportUrl} target="_blank" rel="noreferrer" className="rounded-xl bg-white/10 px-3 py-2 text-center text-xs font-semibold hover:bg-white/20">XLSX</a>
          <a href={workbook.fileUrl} target="_blank" rel="noreferrer" className="rounded-xl border border-white/20 px-3 py-2 text-center text-xs font-semibold hover:bg-white/10">Sheets</a>
        </div>
      </div>
    );
  }
  ...
  if (!isLoaded && !E2E_DEMO) {
    return <main className="flex min-h-[100dvh] items-center justify-center bg-slate-950 p-6 text-white"><p>Loading English OS...</p></main>;
  }
    ...
    return (
      <main className="flex min-h-[100dvh] items-center justify-center bg-slate-950 p-4 text-white">
        <div className="w-full max-w-sm rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
          ...
          <SignInButton mode="modal"><button className="mt-6 w-full rounded-2xl bg-blue-600 px-4 py-3 font-semibold hover:bg-blue-500">Sign in</button></SignInButton>
        </div>
      </main>
    );
  }
      ...
      <div className="mx-auto flex min-h-[100dvh] max-w-7xl flex-col px-3 py-3 sm:px-4 lg:px-6 lg:py-5">
        <header className="mb-3 rounded-[2rem] border border-white/10 bg-slate-950/80 p-4 shadow-2xl shadow-black/30 lg:sticky lg:top-3 lg:z-20 lg:backdrop-blur">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 lg:grid-cols-4">
            <button type="button" onClick={startTodayClass} disabled={loading || !activeStudyUnit} className="rounded-2xl bg-blue-600 px-3 py-3 text-xs font-bold text-white hover:bg-blue-500 disabled:opacity-50">Empezar explicación</button>
            <button type="button" onClick={requestHint} disabled={loading || !activeStudyUnit} className="rounded-2xl border border-yellow-500/50 bg-yellow-500/10 px-3 py-3 text-xs font-bold text-yellow-100 hover:bg-yellow-500/20 disabled:opacity-50">Dame una pista</button>
            <button type="button" onClick={requestUnitGrammar} disabled={loading || !activeStudyUnit} className="rounded-2xl border border-emerald-500/40 bg-emerald-500/10 px-3 py-3 text-xs font-bold text-emerald-100 hover:bg-emerald-500/20 disabled:opacity-50">Gramática</button>
            <button type="button" onClick={requestUnitVocabulary} disabled={loading || !activeStudyUnit} className="rounded-2xl border border-cyan-500/40 bg-cyan-500/10 px-3 py-3 text-xs font-bold text-cyan-100 hover:bg-cyan-500/20 disabled:opacity-50">Vocabulario</button>
          </div>
        ...
        <div className="grid flex-1 gap-3 lg:grid-cols-[minmax(0,1fr)_340px]">
          <section className="flex min-h-[70dvh] min-w-0 flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950/70 shadow-2xl lg:h-[calc(100dvh-245px)]">
            ...
            <footer className="sticky bottom-0 z-10 border-t border-white/10 bg-slate-950/95 p-3 backdrop-blur sm:p-4">
              <div className="flex flex-col gap-2 sm:flex-row">
                <textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); sendMessage(); } }} placeholder="Escribe tu respuesta en inglés o pide una explicación..." className="min-h-24 flex-1 resize-none rounded-2xl border border-slate-700 bg-slate-900 p-3 text-base text-white outline-none focus:border-blue-500" />
                <button onClick={() => sendMessage()} disabled={loading || !input.trim()} className="rounded-2xl bg-blue-600 px-6 py-3 font-semibold hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50 sm:w-36">{loading ? "..." : "Enviar respuesta"}</button>
              </div>
          ...
          <aside className="space-y-3 lg:h-[calc(100dvh-245px)] lg:overflow-y-auto">
            <section className="rounded-3xl border border-white/10 bg-slate-950/70 p-4">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                <button type="button" onClick={() => setStudyUnit(currentUnit)} disabled={!currentUnit} className="rounded-2xl border border-slate-700 px-3 py-2 text-xs font-semibold hover:bg-slate-800 disabled:opacity-50">Usar actual</button>
                <button type="button" onClick={startTodayClass} disabled={loading || !activeStudyUnit} className="rounded-2xl bg-blue-600 px-3 py-2 text-xs font-semibold hover:bg-blue-500 disabled:opacity-50">Clase</button>
              </div>
            </section>
            ...
            <section className="rounded-3xl border border-white/10 bg-slate-950/70 p-4">
              ...
              <div className="mt-3 grid gap-2">
                <button type="button" onClick={() => createWorkbook("grammar")} disabled={grammarWorkbookLoading || !activeStudyUnit || E2E_DEMO} className="rounded-2xl bg-emerald-600 px-3 py-3 text-sm font-semibold text-white hover:bg-emerald-500 disabled:opacity-50">{grammarWorkbookLoading ? "Generando..." : `Excel gramática · ${activeStudyUnitLabel}`}</button>
                <button type="button" onClick={() => createWorkbook("vocabulary")} disabled={vocabularyWorkbookLoading || !activeStudyUnit || E2E_DEMO} className="rounded-2xl bg-cyan-600 px-3 py-3 text-sm font-semibold text-white hover:bg-cyan-500 disabled:opacity-50">{vocabularyWorkbookLoading ? "Generando..." : `Excel vocabulario · ${activeStudyUnitLabel}`}</button>
                <button type="button" onClick={requestUnitGrammar} disabled={loading || !activeStudyUnit} className="rounded-2xl border border-emerald-700 px-3 py-3 text-sm font-semibold text-emerald-100 hover:bg-emerald-950 disabled:opacity-50">Guía de gramática en chat</button>
                <button type="button" onClick={requestUnitVocabulary} disabled={loading || !activeStudyUnit} className="rounded-2xl border border-cyan-700 px-3 py-3 text-sm font-semibold text-cyan-100 hover:bg-cyan-950 disabled:opacity-50">Guía de vocabulario en chat</button>
              </div>
            ...
            <section className="rounded-3xl border border-white/10 bg-slate-950/70 p-4">
              ...
              <select value={activeAgentId} onChange={(event) => setActiveAgentId(event.target.value as AgentId)} className="mt-3 w-full rounded-2xl border border-slate-700 bg-slate-950 px-3 py-3 text-sm text-white outline-none focus:border-blue-500">
                {SPECIALIST_AGENTS.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}
              </select>
              <div className="mt-3 grid grid-cols-3 gap-2">
                {SPECIALIST_AGENTS.map((agent) => (
                  <button key={agent.id} type="button" onClick={() => { setActiveAgentId(agent.id); sendAgentMessage(agent.defaultPrompt); }} disabled={agentLoading} className="rounded-2xl border border-slate-700 px-2 py-3 text-xs font-semibold text-slate-200 hover:bg-slate-800 disabled:opacity-50">{agent.shortName}</button>
                ))}
              </div>
              ...
              <div className="mt-3 space-y-3">
                {resources.map((resource) => (
                  <div key={resource.resourceId} className="rounded-2xl border border-slate-800 bg-slate-950 p-4">
                    <div className="mb-2 flex items-start justify-between gap-2"><h3 className="text-sm font-semibold text-slate-100">{resource.title}</h3><span className="rounded-full border border-slate-700 px-2 py-1 text-[10px] uppercase text-slate-400">{resource.type}</span></div>
                    ...
                    {resource.type === "video" && resource.embedUrl && <div className="mb-3 aspect-video overflow-hidden rounded-2xl border border-slate-800 bg-black"><iframe src={resource.embedUrl} title={resource.title} className="h-full w-full" allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen /></div>}
                    <div className="grid grid-cols-2 gap-2"><a href={resource.url} target="_blank" rel="noreferrer" className="rounded-2xl border border-slate-700 px-3 py-2 text-center text-sm font-semibold hover:bg-slate-800">Abrir</a><button type="button" onClick={() => requestResourcePractice(resource)} disabled={loading} className="rounded-2xl bg-blue-600 px-3 py-2 text-sm font-semibold hover:bg-blue-500 disabled:opacity-50">Practicar</button></div>
                  </div>
                ))}
              </div>
            </section>
