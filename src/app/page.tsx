...
export default function EnglishOSV02Home() {
  ...
  async function loadV02() {
    ...
    try {
      const response = await fetch("/api/english-os/v02", {
        method: "POST",
      ...
      if (!response.ok || !result.ok) {
        throw new Error(result.error || "Could not load English OS v0.2.");
      }
  ...
  async function submitPractice() {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok || !result.ok) {
        throw new Error(result.error || "Could not analyze practice.");
      }
  ...
  async function approvePractice() {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok || !result.ok) {
        throw new Error(result.error || "Could not approve practice.");
      }

      setApprovalMessage("Practice approved. The learner was not advanced automatically.");
    } catch (error) {
  ...
  async function runSelfTest() {
    setMessage("");
    const response = await fetch("/api/english-os/v02?selfTest=1", { cache: "no-store" });
    const result = await response.json();
    setSelfTest(result);
  }
  ...
  function saveSession() {
    ...
    const key = `english-os-v02-session:${email}`;
    const existing = JSON.parse(localStorage.getItem(key) || "[]") as unknown[];
    localStorage.setItem(key, JSON.stringify([saved, ...existing].slice(0, 20)));
    setApprovalMessage("Session saved locally. You no longer need to type “log the session”.");
  }
      ...
      <div className="mx-auto max-w-7xl p-5 md:p-8 space-y-6">
        <header className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl md:text-4xl font-bold">Adaptive Learning UX</h1>
        ...
        {data?.warnings && data.warnings.length > 0 && (
          <Alert tone="warning">
            MCP warnings: {data.warnings.join(" | ")}. The v0.2 pedagogical fallback is active.
          </Alert>
        )}
        ...
        {!data && (
          <section className="rounded-3xl border border-slate-800 bg-slate-900 p-6">
            <p className="text-slate-300">
              {loading ? "Loading learner context from English OS MCP..." : "No English OS v0.2 data loaded yet."}
            </p>
          </section>
        )}
        ...
        {data && activeView === "class" && (
          <CurrentClassPlayer data={data} onPractice={() => setActiveView("practice")} />
        )}
    ...
    <section className="space-y-6">
      <div className="rounded-3xl border border-sky-500/40 bg-gradient-to-br from-slate-900 to-slate-950 p-6 md:p-8">
        ...
        <h2 className="mt-3 text-3xl font-bold">
          {data.learner.name}, today your focus is:
        </h2>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-4">
        <CurrentPositionCard data={data} />
        <TodayFocusCard data={data} />
        <NextRecommendedActionCard data={data} />
...
function CurrentPositionCard({ data }: { data: V02Data }) {
  return (
...
function TodayFocusCard({ data }: { data: V02Data }) {
  return (
...
function NextRecommendedActionCard({ data }: { data: V02Data }) {
  return (
...
function ProgressSnapshot({ data }: { data: V02Data }) {
  return (
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1.45fr_0.85fr]">
        <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6 space-y-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Panel title="Video / Audio / Material">
              <p className="text-slate-300">
                Use this class as an integrated review. Watch or read the available material, then produce a B2 business advice answer.
              </p>
              <p className="mt-3 text-sm text-slate-500">Consulted class: none selected outside the current review path.</p>
            </Panel>

            <Panel title="Notes / Transcript">
              <p className="text-slate-300">
                Your answer must combine advice, contrast, prioritization, and consequence. This class is reviewing—not advancing automatically.
              </p>
            </Panel>
          </div>
        ...
        <aside className="rounded-3xl border border-slate-800 bg-slate-900 p-6 space-y-5">
          <Panel title="AI Coach Instructions">
            <p className="text-slate-300">
              First correct quickly. Then explain the rule. End with a retry prompt and automatic session summary.
            </p>
          </Panel>
          ...
          <Panel title="Key Language">
            <div className="flex flex-wrap gap-2">
              {data.currentClass.keyLanguage.map((item) => (
                <span key={item} className="rounded-full bg-slate-800 px-3 py-1 text-sm text-slate-200">
                  {item}
                </span>
              ))}
            </div>
  ...
  return (
    <article className="rounded-2xl border border-slate-800 bg-slate-950 p-5 space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <button onClick={onMastered} className="rounded-lg border border-slate-700 px-3 py-1 text-sm hover:bg-slate-800">
          Mark as mastered
        </button>
      </div>
      ...
      <div>
        ...
        <ul className="mt-2 space-y-2 text-slate-300">
          {mistake.examples.map((example) => (
            <li key={example} className="rounded-xl bg-slate-900 p-3">
              {example}
            </li>
          ))}
        </ul>
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <button onClick={onPractice} className="rounded-xl bg-sky-400 px-4 py-2 font-semibold text-slate-950 hover:bg-sky-300">
          Practice
        </button>
        <button onClick={onPractice} className="rounded-xl border border-slate-700 px-4 py-2 font-semibold hover:bg-slate-900">
          Speaking retry
        </button>
      </div>
    </article>
    ...
    <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_0.85fr]">
      <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6 space-y-5">
        <div>
          ...
          <p className="mt-2 text-slate-400">
            The analyzer uses the current class, recurring mistakes, active vocabulary, CEFR target, and business reasoning objective.
          </p>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            className="mt-2 min-h-36 w-full rounded-2xl border border-slate-700 bg-slate-950 p-4 text-slate-100 outline-none focus:border-sky-400"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="The way I see it, you ought to..."
            className="mt-2 min-h-44 w-full rounded-2xl border border-slate-700 bg-slate-950 p-4 text-slate-100 outline-none focus:border-sky-400"
          />
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <button
            ...
            onClick={onSubmit}
            className="rounded-xl bg-sky-400 px-5 py-3 font-bold text-slate-950 hover:bg-sky-300 disabled:opacity-50"
          >
        ...
        <div className="grid gap-2 md:grid-cols-3">
          {TEST_INPUTS.map((item) => (
            <button
              key={item.label}
              onClick={() => onUseTestInput(item.value)}
              className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-left text-sm text-slate-300 hover:border-sky-500"
            >
              {item.label}
            </button>
          ))}
      ...
      <aside className="rounded-3xl border border-slate-800 bg-slate-900 p-6 space-y-5">
        <Panel title="Active vocabulary to recycle">
          <div className="flex flex-wrap gap-2">
            {data.activeVocabulary.map((item) => (
              <span key={item} className="rounded-full bg-slate-800 px-3 py-1 text-sm text-slate-200">
                {item}
              </span>
            ))}
        ...
        <Panel title="Approval rule">
          <p className="text-slate-300">
            Grammar, vocabulary, and business reasoning must be approved. Opening content or submitting a weak answer never advances the learner.
          </p>
        </Panel>
      ...
      <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6 space-y-5">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <Panel title="Quick correction">
            <p className="text-slate-100">{evaluation.quickCorrection}</p>
          </Panel>
          <Panel title="Detailed explanation">
            <p className="text-slate-300">{evaluation.detailedExplanation}</p>
          </Panel>
        </div>

        <Panel title="Retry prompt">
          <p className="text-slate-300">{evaluation.retryPrompt}</p>
        </Panel>

        {result.mcpAnalysis?.reply && (
          <Panel title="MCP conversation analysis">
            <p className="whitespace-pre-wrap text-sm leading-6 text-slate-300">{result.mcpAnalysis.reply}</p>
          </Panel>
        )}

        {result.mcpAnalysisError && (
          <Alert tone="warning">MCP analysis warning: {result.mcpAnalysisError}</Alert>
        )}
      </div>
      ...
      <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6 space-y-5">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Panel title="Next action">
            <p className="text-slate-300">{evaluation.sessionSummary.nextAction}</p>
          </Panel>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-5">
          <button onClick={onSave} className="rounded-xl bg-emerald-400 px-4 py-3 font-bold text-slate-950 hover:bg-emerald-300">
            Save Session
          </button>
          <button onClick={onPracticeAgain} className="rounded-xl border border-slate-700 px-4 py-3 font-bold hover:bg-slate-900">
            Practice Again
          </button>
          <button onClick={onContinueClass} className="rounded-xl border border-slate-700 px-4 py-3 font-bold hover:bg-slate-900">
            Continue Current Class
          </button>
          <button onClick={onReviewMistakes} className="rounded-xl border border-slate-700 px-4 py-3 font-bold hover:bg-slate-900">
            Review Mistakes
          </button>
          <button
            onClick={onApprove}
            disabled={!evaluation.eligibleForApproval}
            className="rounded-xl bg-sky-400 px-4 py-3 font-bold text-slate-950 hover:bg-sky-300 disabled:cursor-not-allowed disabled:opacity-40"
          >
            Approve Practice
          </button>
        </div>

        {approvalMessage && <Alert tone={approvalMessage.includes("approved") ? "success" : "warning"}>{approvalMessage}</Alert>}
      </div>
    </section>
  );
}
...
function DashboardCard({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-900 p-5 space-y-3">
